...
app.controller('AuthCtrl', function ($scope, $http, TokenStorage) {
	...
	$scope.init = function () {
		$http.get('/cynosure_jwt_server/api/users/current').success(function (user) {
	...
	$scope.login = function () {
		$http.post('/cynosure_jwt_server/api/login', { username: $scope.username, password: $scope.password })
			...
			$scope.token = JSON.parse(atob(TokenStorage.retrieve().split('.')[0]));
                        ...
                        $http.get('/cynosure_jwt_server/admin/home').success(function (data) {
                            alert("admin home " + data);
                        }).error(function(){alert("some error in get")})
		}).error(function(){
                    alert("some error")
                }) 
	};
